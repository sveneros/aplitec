...
function ObtenerCategorias1(){
    ...
    if ( $.fn.dataTable.isDataTable('#basic-1') ) {
        table=$('#basic-1').DataTable().destroy();
    }
    var localData=JSON.parse(localStorage.getItem('sml2020_categorias'));
    var html='';
    $.each(localData,function(key,value){
        var est='';var edi='';var img='';
    if(value.estado=='V'){
        est='<span class="badge rounded-pill bg-success badge-notification">HABILITADO</span>';
        edi='<button class="btn btn-primary" onclick=\"Editar(\''+value.id+'\')\" data-bs-toggle="modal" data-original-title="test" data-bs-target="#exampleModal"> <i class="fa fa-pencil"></i> </a>';
        
        html+='<tr role="row" class="odd"><td class="sorting_1">'+value.descripcion+'</td><td>'+est+'</td><td>'+edi+'</td></tr>';          
    }
